...
class State():
    ...
    def calculate_wait(self, choice: int) -> int:
        max_turns_required = 0
        for i, cost in enumerate(self.blueprint[choice]):
            if cost == 0:
                continue
    ...
    def build_robot(self, choice: int):
        # Subtract resources, add a robot
        robot_cost = self.blueprint[choice]
...
def run(cur_state: State) -> int:
    ...
    for choice in build_choices:
        # make a new copy so we can explore that
        new_state = deepcopy(cur_state)
        # plus one because it takes a turn to build the robot
        turns_required = new_state.calculate_wait(choice) + 1
...
def get_most_geodes(blueprint):
    """Get the answer for the given blueprint"""
    global state_cache
    cur_state = State(blueprint)
    state_cache = {}
...
def main():
    ...
    for blueprint_number, cur_blueprint in blueprints.items():
        ...
        num_geodes = get_most_geodes(cur_blueprint)
